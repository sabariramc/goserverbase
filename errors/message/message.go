...
func CreateErrorMessage(ctx context.Context, serviceName, errorCode string, err error, stackTrace string, errorData interface{}, alertType string) *utils.Message {
	...
	msg.AddPayload("identity", utils.Payload{"entity": trace.ExtractUserIdentifier(ctx).GetPayload()})
	msg.AddPayload("correlation", utils.Payload{"entity": trace.ExtractCorrelationParam(ctx).GetPayload()})
