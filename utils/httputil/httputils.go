...
func NewHttpClient(log *log.Logger, logConfig LogConfig, retryMax int, retryWaitMin, retryWaitMax time.Duration) *HttpClient {
	...
	c := &HttpClient{Client: &http.Client{Transport: t}, log: log.NewResourceLogger("HttpClient"), LogConfig: logConfig, RetryMax: retryMax, RetryWaitMin: retryWaitMin, RetryWaitMax: retryWaitMax, CheckRetry: retryablehttp.DefaultRetryPolicy, Backoff: retryablehttp.DefaultBackoff}
	return c
}
