...
func TestS3(t *testing.T) {
	...
	err = s3Client.GetFile(ctx, s3Bucker, path, "../testfile/test.pdf")
	if err != nil {
		t.Fatal(err)
	}
	_, err = s3Client.CreatePresignedURLGET(ctx, s3Bucker, path, 10*60)
	if err != nil {
		t.Fatal(err)
	}
	path = fmt.Sprintf("dev/temp/goserverbasetest/%v.pdf", uuid.NewString())
	_, err = s3Client.CreatePresignedURLPUT(ctx, s3Bucker, path, 10)
	if err != nil {
		t.Fatal(err)
	}
}
